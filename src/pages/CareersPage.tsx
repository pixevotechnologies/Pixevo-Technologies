...
import { CareersSkeleton } from '../components/Skeletons';
import {
  Briefcase,
  MapPin,
  Clock,
...
export const CareersPage: React.FC<CareersPageProps> = ({
  ...
  const handleDepartmentChange = (dept: string) => {
    if (dept === departmentFilter) return;
    setIsLoading(true);
            ...
            {perks.map((p) => {
              ...
              return (
                <StaggerItem key={p.title}>
                  <div className="h-full p-6 rounded-2xl bg-slate-900/40 border border-slate-800 space-y-3">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <button
                  ...
                  disabled={isLoading}
                  title={language === 'ar' ? 'إعادة تحميل الشواغر' : 'Simulate fetch & reload careers'}
                  className="p-2 rounded-full bg-slate-950 text-slate-400 hover:text-white hover:bg-slate-900 border border-slate-800 transition-all cursor-pointer disabled:opacity-50 shrink-0"
                >
